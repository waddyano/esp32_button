...
#if CONFIG_BT_HID_HOST_ENABLED
static void handle_bt_device_result(struct esp_bt_gap_cb_param_t::disc_res_param *disc_res)
{
...
static bool handle_ble_device_result(struct esp_ble_gap_cb_param_t::ble_scan_result_evt_param *scan_rst)
{
    uint16_t uuid = 0;
    ...
    if (uuid == ESP_GATT_UUID_HID_SVC || (appearance >> 6) == 0xf) {
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT: {
            if (handle_ble_device_result(&scan_result->scan_rst))
            {
                ESP_LOGV(TAG, "BLE GAP EVENT FOUND: %d", scan_result->scan_rst.num_resps);
